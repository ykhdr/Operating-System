...
void* ascending_thread(void* data) {
    ...
    while (1) {
        ...
        node_t *curr2, *tmp;
        while (1) {
            if (curr != NULL && pthread_spin_trylock(&curr->sync) == 0) {
                if (curr->next != NULL && pthread_spin_trylock(&curr->next->sync) == 0) {
                    int pair_count = 0;
                    curr2 = curr->next;
...
void* swap_thread(void* data) {
    ...
    while (1) {
        ...
        while (1) {
            if (curr1 != NULL && pthread_spin_trylock(&curr1->sync) == 0) {
                if (curr1->next != NULL && pthread_spin_trylock(&curr1->next->sync) == 0) {
                    if (curr1->next->next != NULL && pthread_spin_trylock(&curr1->next->next->sync) == 0) {
                        curr2 = curr1->next;
